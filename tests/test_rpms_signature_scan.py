"""Test rpms_signature_scan.py end-to-end"""
...
from rpms_signature_scan import rpms_signature_scan
from rpms_signature_scan.rpms_signature_scan import (
    ImageProcessor,
    ProcessedImage,
    generate_output,
    generate_results,
...
class TestImageProcessor:
    ...
    def test_call_signed_rpms_keys_getter_exception(  # pylint: disable=too-many-arguments
        ...
        """Test ImageProcessor exception in signed_rpms_keys_getter"""
        stderr = "Failed to run command"
...
class TestMain:
    """Test call to rpms_signature_scan.py main function"""
    ...
    def mock_image_processor(self, monkeypatch: MonkeyPatch) -> MagicMock:
        ...
        monkeypatch.setattr(rpms_signature_scan, ImageProcessor.__name__, mock)
        return mock
    ...
    def create_generate_output_mock(
        ...
        def _mock_generate_output(with_failures: bool = False) -> MagicMock:
            ...
            monkeypatch.setattr(rpms_signature_scan, generate_output.__name__, mock)
            return mock

        return _mock_generate_output

    @pytest.fixture()
    def create_generate_results_mock(
        self, monkeypatch: MonkeyPatch
    ) -> Callable[[], MagicMock]:
        """Create a generate_results mock"""

        def _mock_generate_results() -> MagicMock:
            """Monkey-patched generate_results"""
            mock = create_autospec(generate_results, return_value={"results": "res"})
            monkeypatch.setattr(rpms_signature_scan, generate_results.__name__, mock)
            return mock

        return _mock_generate_results
    ...
    def test_main(  # pylint: disable=too-many-arguments
        ...
        create_generate_results_mock: MagicMock,
        ...
        generate_results_mock = create_generate_results_mock()
        rpms_signature_scan.main(  # pylint: disable=no-value-for-parameter
            args=[
                "--input",
                "img1",
                "--fail-unsigned",
        ...
        if has_errors:
            assert temp_folder.joinpath("status").read_text() == "ERROR"
        else:
            assert temp_folder.joinpath("status").read_text() == "SUCCESS"
        assert mock_image_processor.return_value.call_count == 1
        assert temp_folder.joinpath("results").read_text() == '{"results": "res"}'
    ...
    def test_main_fail_on_unsigned_rpm_or_errors(
        ...
        """Test call to rpms_signature_scan.py main function fails
        when whe 'fail-unsigned' flag is used and there are unsigned RPMs
        """
        fail_unsigned: bool = True
        generate_output_mock = create_generate_output_mock(with_failures=fail_unsigned)
        generate_results_mock = create_generate_results_mock()
        ...
        generate_results_mock.assert_called_once()
